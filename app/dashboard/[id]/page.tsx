import { getChatWithUser } from "@/actions/chat"
...
export default async function Chat({ params }: { params: Promise<{ id: string }> }) {
    ...
    const user = await getChatWithUser(Number(id))
    if (!user) notFound()
            ...
            <div className="flex items-center justify-between px-[20px] py-[20px] gap-[20px] border-b border-b-neutral-200">
                <div className="flex items-center justify-center gap-[15px]">
                    <div className="relative">
                        <Image src={"/avatars/2.svg"} alt={'avatar'} width={55} height={55} />
                ...
                <input type="text" className="outline-0" placeholder="Поиск по сообщениям" />
            </div>
            </div>
            <div className="flex flex-col px-[20px] py-[20px] h-full">
                <div className="flex flex-col gap-[10px]">
                    <div className="inline-flex items-center max-w-1/2 break-words w-max gap-[10px] rounded-[12px] px-[10px] py-[8px] bg-neutral-200">
                        <p>message-in</p>
                        <small className="text-neutral-500">10:11</small>
                    </div>
                    <div className="inline-flex ml-auto items-center max-w-1/2 break-words w-max gap-[10px] rounded-[12px] px-[10px] py-[8px] bg-blue-200">
                        <p>message-out</p>
                        <small className="text-blue-500">10:11</small>
                    </div>
                </div>
            </div>
